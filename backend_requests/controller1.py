        ...
        if filter(lambda v: v.isAvailable,vehicles) == []: # the piority will increase by ratio when customers are waiting
            ...
            for v in free_vehicles:
                if v.id in vehicle_queue:
                    ...
                    v.customerId = waiting_customer[index]
            ...
            if free_vehicles == []:
                ...
                end_positions,distance_to_go = base_distance_calculator(available_vehicles,awaiting_customers)
                ...
                for i,v_index in enumerate(v_indices):
                    if vehicles[available_vehicles_index[v_index]].isAvailable:
                        vehicles[available_vehicles_index[v_index]].customerId = customers[awaiting_customers_index[c_indices[i]]].id
                    else:
